...
minimumHostCount=-32768; maximumHostCount=32767;  %Set the minimum and maximum host counts

% Create the integer indices that serve as input to the table lookup.
hostTableLookupDACInputVector=minimumHostCount:maximumHostCount;
...
hostTableLookupVoltageOutputVector(:,channelCount)=round(interp1(hostVoltageRepresentationAtCalibrationMulti(numCalPoints,channelCount),dacAtCalibration(numCalPoints),hostTableLookupDACInputVector, "linear"));
% Remove signed component from results for now
hostTableLookupVoltageOutputVector(:,channelCount)=hostTableLookupVoltageOutputVector(:,channelCount) + 32768;

TableLookupOutput=hostTableLookupVoltageOutputVector(:,channelCount);
