...
)
{
    ...
    for (dwSlaveIdx = 0; dwSlaveIdx < emGetNumConnectedSlaves(pAppContext->dwInstanceId); dwSlaveIdx++)
    {
...
)
{
    ...
    for (dwSlaveIdx = 0; dwSlaveIdx < emGetNumConfiguredSlaves(pAppContext->dwInstanceId); dwSlaveIdx++)
    {
        EC_T_WORD           wAutoIncAddress = (EC_T_WORD)(0 - dwSlaveIdx);
        EC_T_CFG_SLAVE_INFO oCfgSlaveInfo;
...
\return PC Text.
*/
EC_T_CHAR* SlaveProdCodeText(T_eEtherCAT_Vendor eVendorId, T_eEtherCAT_ProductCode eProductCode)
{
    EC_T_CHAR* pRet = EC_NULL;
